...
| 层级 | 指标 | 目标值（首版） | 说明 |
|---|---|---|---|
| **北极星** | **演示通过率**：按 §5 的 12 步演示脚本连续跑通且无假反馈 | 12/12，3 次重跑稳定 | 作品集场景专用指标，替代 DAU |
...
> 脚本清单、"已知限制"一节。新增 `DEMO.md`：预热 + LLM 验活 + 演示账号 + 12 步演示脚本 +
...
- 加 `DEMO.md`：公开访问地址、种子测试账号、12 步演示脚本、**预热说明**（Render 免费档休眠 + 本地嵌入模型首次加载耗时 → 演示前先打一次冷启动请求）。
...
>
> **结论有两个，第二个比第一个重要**：
> 1. 0.50 是免费的——过滤 20% 无关查询且不伤任何真命中，可作为默认值；
>    但**暂时仍留 `RAG_MIN_SCORE=0`**：0.5 会让约 20% 的无关查询返回空上下文，而空上下文目前没有专门分支，
>    会渲染出一句"找到以下相关内容："加空白。这一改动和空上下文兜底一起做（即 P1-2），不单独上线。
> 2. 但**单靠 cosine 阈值做不到可靠拒答**。负样本 top1 分数区间 0.426~0.728，正样本 0.532~0.762，两者大面积重叠：
>    想拒掉 2/3 的噪声就得牺牲 15% 的真命中，这不是可接受的交换。P1-2 的"知识库无相关内容"必须换信号
>    （先让 LLM 判断检索片段是否真能支撑回答，或改用相对分数间隔），不能只调 `RAG_MIN_SCORE`。
>
