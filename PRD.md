...
| 层级 | 指标 | 目标值（首版） | 说明 |
|---|---|---|---|
| **北极星** | **演示通过率**：按 §5 的 17 步演示脚本连续跑通且无假反馈 | 17/17，3 次重跑稳定 | 作品集场景专用指标，替代 DAU |
...
> ~~服务端题库分页/筛选~~ 已在 P2 性能里做完，并且**这一轮补上了前端接线**：
> 之前只有服务端支持 `?category=&difficulty=&limit=`，`api.questions.getAll` 仍然先拉全量再本地筛，
> 等于"接口有、没人用"。现在练习页选了分类就真的只传分类。
...
>
> **P2 仍未做**：无。i18n 那一行经评估决定不做，理由见下。
>
> **进度补记（2026-09-23，i18n 行：决定不做，并说明在什么条件下会翻案）**
>
> 当初这行是照竞品清单抄的（"README 双语 → UI 双语文案集中"）。真去做之前先量了一下：
> - 依赖侧干净：`package.json` 里没有任何 i18n 库，不存在"装了没用"的僵尸依赖要清；
> - **语料是纯中文的**：`documents.json` 307 条全部是中文 AI 岗面试材料，题库 255 题、文章 27 篇亦然。
>
> 于是问题不在"能不能抽出 500 条 UI 文案"，而在**抽出来给谁看**：
> 一个英文壳的界面，包着只有中文的题库、文章、面试题和 AI 回答 —— 访客把语言切到 English，
> 结果是导航变英文、内容仍是中文，这比纯中文界面更不像个成品。
> 而本项目定位是"给中文 AI 岗面试官看的作品集"，双语不是需求，是清单惯性。
>
> 所以这一行按"决定不做"关闭，而不是"没做完"。翻案条件写清楚，免得下次又当 TODO 抄回来：
> ① 语料出现第二语言（比如同一套题配英文讲解），或 ② 目标用户扩到非中文求职者。
> 届时按本次主题开关的同一个路子做即可：文案集中到一个模块 → 语言作为一组变量翻，
> 而不是在组件里散落字符串。**关键前置是先有"单一声明点"，这件事已经在颜色和角色色上做完了。**
>
...
>
> **进度补记（2026-09-23，性能行的尾巴：接口有、没人用）**：上一段做完服务端筛选后，
> 我按 PRD 的说法以为这一项结了。真在浏览器里看网络才发现 `api.questions.getAll` 依旧
> 先 `fetch('/api/questions')` 拉全量、再在 JS 里 filter/slice —— 服务端那三个 query 参数一个都没人传。
> 现在带 category/difficulty/limit 时走服务端，返回体从 693KB 降到 249KB（选 RAG 那一类，68 题）；
> 不选分类仍是全量，那是"全部分类"本来就该拿到的东西，没假装变快。
> 命中总数走 `X-Total-Count`，而跨域时浏览器默认只让前端读 6 个安全响应头 ——
> 第一次实测读回来是 `null`，补了 `exposedHeaders` 才真拿到。这类"服务端发了、前端看不见"的坑，
> 只看代码是查不出来的，得看请求。
> `search` 与 `page` 服务端不支持，这两种情况显式回落老路径（宁可多拉一次也不要筛错结果）；
> 新增一条单测钉住"`total` 是截断前的命中数而不是本次返回条数"，否则前端会以为 RAG 只有 2 题。
> 用例数 32 → 33。
>
...
>
> **进度补记（2026-09-23，对比度：真浏览器顺手抓出来的一批）**
>
> 截图能看"顺不顺眼"，看不出"读不读得清"。所以给同一套脚本加了一项：遍历每个**叶子文字节点**，
> 用 `getComputedStyle` 拿前景色、向上解析真正垫在它下面的背景色，按 WCAG 相对亮度公式算对比度。
> 阈值先取 3.2:1（低于 AA 的 4.5，也低于非文本的 3.0，只当筛子不当结论）。
>
> **基线：20 个页面×视口组合里，54 类文字不达标。** 其中三条是硬伤：
> - 首页主 CTA「立即体验」白字压在 `#06d6a0` 上 = **1.89:1**，等于没有；同一条规则覆盖 25 处薄荷/品红渐变按钮。
> - 登录页顶部横幅「欢迎回来」同样白字压薄荷，`text-white/80` 的副标题一起消失。
> - `text-ghost` 用在正文上（"AI 将为你生成优化建议"）= **1.65:1**；`text-faint` 当正文用有 87 处，压在卡片上 2.6~2.9:1。
>
> 修法按"能一处改完就不改 87 处"的顺序：
> 1. **语义分层**：className 里同时出现字号工具类的，说明那是给人读的文字而不是图标，
>    一律 `text-faint`/`text-ghost` → `text-muted`（89 处，纯字符串替换，diff 是 89 增 89 删的同行改动）。
> 2. **改 token 值**：`faint` 从 `#5a5a6e` 提到 `#7d7d92`（图标与漏网提示的兜底），
>    `ghost` 在注释里写死"只用于描边与装饰"。这一步只动 `tailwind.config.js` 一行，全站跟着走 ——
>    上一轮 token 收敛的收益在这里兑现了：以前要改 156 处字面 hex。
> 3. **薄荷底改深字**：`text-white` → `text-ink`（1.89:1 → 10.47:1）。双色渐变（薄荷→紫）两个方向都不达标时，
>    取"最不利端"算：深字最差 3.47:1，白字最差 1.89:1，所以选深字。这也和导航选中态（绿底深字）一致。
> 4. `body { color-scheme: dark }`：`<select>` 的下拉箭头和原生滚动条之前由浏览器按亮色画一块白，
>    在暗色卡片上很扎眼，一行声明让原生控件跟着配色走。
>
> **复测：0 类低于 3.2:1，20 个组合全部无横向溢出。** 因为"0"这个词这次已经骗过我三回，
> 补了一个阳性对照：在页面里注入"白底白字"和"薄荷渐变上的白字"两个已知坏样本，
> 探针分别报回 `rgb(255,255,255)` 与 `rgb(6,214,160)`（渐变里最不利的色标），确认它还会响。
>
> 探针本身错过两次，都记下来：① 渐变写在 `background-image` 里，`background-color` 是透明的，
> 第一版拿祖先底色算，报出"深字压在薄荷按钮上 1.03:1"的假阳性；② 修的时候把所有色标和祖先底色混在一起
> 取最接近的，等于自己把假阳性留下了 —— 正确做法是**遇到渐变层就只看它的色标**，渐变是盖住底色的。
>
> 一次操作事故也记在这里：我用 `node -e "..."` 内联跑一个"看起来是无操作"的正则时，
> 反引号被 Git Bash 当成命令替换，正则退化成"删掉所有单词字符"，**52 个源文件被清空标识符**。
> 因为改动全在最后一次提交之后，`git stash push -- src` 取回干净状态、tsc 与 build 复验后丢弃，零损失。
> 教训：往源码上动批量替换，一律写成脚本文件用 Write 落盘再跑，不要塞进 shell 的 `-e`。
>
> **进度补记（2026-09-23，主题开关）**：做了，但 PRD 当初那句"低成本高可见度"是错的。
>
> 真正的成本不在"加个开关"，在于**这套配色原本只有一套语义角色**。分两步走：
>
> **第一步（纯工程，零视觉变化）**：12 个语义色从写死的 hex 改成 `rgb(var(--c-x) / <alpha-value>)`，
> 值集中在 `index.css` 的 `:root` 与 `html.light` 两处；`--color-*` 那组老变量改成引用同一批三元组，
> 于是调色板只剩一个声明点。验收不是"看着没变"，而是逐条比对产物：
> 12 个类都变成 `rgb(var(--c-…) / …)` 形式，且 12 组三元组与收敛前的 hex 一一对上。
>
> **第二步（亮色）**：换 12 个值 + `color-scheme: light` + 顶栏开关（`useTheme` 早就写好了，
> 只是没人引用；顺手把"跟随系统偏好"改成默认暗色并记住选择，再在 `index.html` 里加首屏前置脚本，
> 否则存了亮色的用户每次先被闪一下暗色）。
>
> 然后审计表就暴露了那套缺失的词汇：**25 处 `text-ink` 压在薄荷按钮上，在亮色下变成了白字** ——
> 因为 `ink` 的语义是"页面底色"，亮色下它是接近白的 #f8f9fb。我上一轮为了修暗色对比度把白字换成 `text-ink`，
> 等于把一个"永远要深"的角色错绑到了"会翻"的令牌上。同理还有 92 处 `text-primary-500`
> （薄荷当文字，压白底 1.7:1）、32 处 `text-rose-400`、17 处 `text-amber-400`、16 处 `text-purple-400`、
> 35 处 `text-white`。
>
> 补了 6 个**角色令牌**（值成对翻，定义在同一个 CSS 里）：
> `brand`（强调文字，暗=薄荷 / 亮=primary-700）、`on-brand`（压在品牌实色上的字，两个主题都固定深色，
> 所以它**不**走变量）、`danger`、`success`、`warning`、`accent-fg`、外加 `info`。
> 难度三色本来就在 `constants/config.ts` 单一定义，改三行就够 —— 但那次 codemod 只扫了 `.tsx`，
> 漏了这个 `.ts`，是审计把它抓出来的。
>
> 手写 CSS 里还藏着 7 处字面 `#06d6a0` 与 3 处 `color: #fff`（导航 pill、移动端菜单），
> 一并换成变量。其中 `color: #fff` 三处不能一把梭：`.pill-label-hover` 要跟着正文色翻，
> 而移动端菜单那两处是压在薄荷上的，必须是深色 —— 按选择器逐行判断，不按字符串全局替换。
>
> **验收：暗色 + 亮色 × 桌面 1280 + 窄屏 390 × 10 个页面 = 20 个组合，
> 低对比文字 0、横向溢出 0、JS 错误 0**；亮色下首页/聊天/管理端/面试报告四张图肉眼看过去成立
> （雷达图的网格线走 `style` 而不是 `stroke` 属性，因为 `var()` 在 SVG 呈现属性里不解析）。
> tsc / eslint(0 error) / vite build / 31 个单测全过。
>
> 一句实话：亮色能收干净，靠的是第一步把颜色搬进变量、以及那台逐节点算对比度的探针。
> 没有这两样，"加个主题开关"大概率会变成"交一个亮色下看不清的站"。
