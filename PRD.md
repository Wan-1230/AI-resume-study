...
| 层级 | 指标 | 目标值（首版） | 说明 |
|---|---|---|---|
| **北极星** | **演示通过率**：按 §5 的 16 步演示脚本连续跑通且无假反馈 | 16/16，3 次重跑稳定 | 作品集场景专用指标，替代 DAU |
| 质量 | 检索 hit-rate@5（评估集 60 条） | ≥ 85% | **当前 73.3%**（P1-1 实测，换 bge-small-zh 后从 46.7% 涨上来；离目标还差 11.7pt，剩下的坑在长句改写类查询） |
| 质量 | 答案引用角标合法率（[n] 指向真实检索到的来源） | ≥ 90% | **当前 100%**：14 条标注查询、29 个角标、越界 0（编号合法性可自动测；"是否真支撑该句"仍需人工抽查） |
| 质量 | 无关查询的拒答率 | ≥ 80% | **实测 20%**（阈值 0.5）。目标本身要改：正负样本 cosine 分布大面积重叠，靠阈值到不了 80%，见 P1-1 结论 2 |
| 使用 | 练习完成率（进入练习→交卷） | ≥ 60% | 需要真实使用者，现在只有自测 |
| 成本 | 单次问答平均 token 与费用 | 有面板可见、可预算 | 管理端已出"检索质量"面板（真实查询数、拒答率、平均耗时、赞踩比）；**token 数没有**：流式响应拿不到 usage，不编 |
...
> 脚本清单、"已知限制"一节。新增 `DEMO.md`：预热 + LLM 验活 + 演示账号 + 16 步演示脚本 +
...
- 加 `DEMO.md`：公开访问地址、种子测试账号、16 步演示脚本、**预热说明**（Render 免费档休眠 + 本地嵌入模型首次加载耗时 → 演示前先打一次冷启动请求）。
...
>
> **未做（截至 P1-1 交付时）**：`chunkSize` 400/60 的 A/B 尚未跑。
> 管理后台"检索质量"页与真实查询分数分布随后随 P1-4 的 `retrieval_log` 一起落地了（见 P1-4 进度）。
...
> **未做**：~~会话列表与持久化、重新生成、赞/踩反馈~~ —— 这三项已于 2026-09-22 补齐，见下面第二段进度。

> **进度补记（2026-09-22）**：对话现在会真的存下来。
> `chat_threads` / `chat_messages` / `retrieval_log` 三张表 + `/api/chat/threads` 一组接口
> （列表、读取、重命名、删除、逐条落库、按条赞/踩）。前端加了历史会话侧栏、新对话、
> 重新生成（替换上一则而不是追加）、赞/踩乐观更新并在失败时回滚。
> 标题取首条用户消息前若干字，省掉一次"起名"的 LLM 调用。
> 未登录仍可正常提问，只是不保存 —— 界面直接写明这一点，不给一个点了没反应的按钮。
>
> 顺带把 P1-1 欠的那块补上：每次问答写一行 `retrieval_log`（只存查询、命中 id/标题/分数、
> 是否拒答、耗时，**不存答案正文**），管理后台新增"检索质量"区：真实查询数、拒答率、
> 平均耗时、赞踩比、top1 分数分桶、最近查询逐条可看。实测面板显示 4 次查询 / 拒答率 25% /
> 平均 5237ms / 赞踩 0-1，数字与库里一致。
> token 用量**没有**：流式响应里拿不到 usage，宁缺不假。
>
> 权限实测：未登录取会话 401；别人读你的会话 404；给用户消息点赞 404；
> `feedback` 传非法值 400；传 null 能清空（`down → null` 读回确认为 null）。
> 一轮真实问答后刷新页面，回答、来源卡片、[n] 角标、赞踩状态全部还原。
...
> **进度（2026-09-22）**：已上线。`POST /api/resume/match` + `ResumePage` 报告面板 + `.docx` 导出。
>
> 做法：把 JD 拆成可判断的要求（只要 JD 里字面写着的，禁止模型补"这类岗位大概还需要"），
> 再逐条回简历找证据，**分数由判定汇总算出来**（命中 1 / 部分 0.5 / 未命中 0，按技能/经验/项目分别汇总），
> 不是让模型直接报一个"匹配度 85%"。声称命中却给不出简历原句的，服务端一律下调为部分命中并标注原因。
>
> 验收实测 —— 同一份简历对两个 JD：
>
> | JD | 总体 | 技能 | 经验 | 项目 | 要求条数 |
> |---|---|---|---|---|---|
> | AI 应用工程师（RAG 方向） | **77%** | 83 | 63 | 100 | 11 |
> | Java 后端（电商方向） | **14%** | 10 | 25 | JD 未涉及 | 7 |
>
> 两份报告的要求清单**重叠 0 条**，可区分；命中项 100% 带简历原句证据（脚本断言通过）。
>
> 缺口旁挂的"可补"用的是站内知识库检索（本地嵌入，不额外花钱）。第一版直接把 top3 标题挂上去，
> 实测发现"精通 MySQL 分库分表"会挂出《RAG 检索优化》—— 知识库只覆盖 AI 岗，cosine 分又不高不低，
> 这种提示纯误导。加了相干闸门：中文二元组 / 英文整词有交集才算相关（6/6 单测符合预期）。
> 重跑后 Java 岗 5 条缺口只剩 1 条挂提示（那条确实提到大模型 API），RAG 岗的相关提示照常保留。
>
> 导出：前端用已经在依赖里躺着但一直没用的 `docx` 生成，**不落服务端**（简历是敏感数据，
> 免费 PaaS 的临时盘也不会清理）。生成的文件按 OOXML 校验过：zip 完整性 OK、22 个条目、
> `word/document.xml` 含标题/分项分数/逐条判定表/待补清单，表格行数与判定条数一致。
> 说明：没能在真 Word 里打开验证，只做到包结构与正文内容级别的正确性。
...
>   所以这里只删不迁 —— 迁到 v4 会动全站样式，当时判断无法做视觉回归，看不到结果的迁移不做。
>   （更正：那个"无法"是错的，本机装得出真浏览器截图，见后面的「视觉验收（真浏览器）」一节。
>   但结论仍然成立：v4 迁移不在 PRD 范围内，收益只是省一层 postcss 配置，不值得为它重排全站样式。）
...
>   没有视觉回归手段时不碰）。
>
> **更正一条我自己写错的"bug"**（2026-09-23）：我先前在进度里断言"产物里 `.sm\:` 一条都没有、
> 源码 70+ 处 `sm:` 全部空写"。那是不成立的 —— 结论来自我自己的搜索串写错了：
> Tailwind 的转义是 `.sm\:px-6`，我却把反斜杠放在了变体名前（`.\sm\:`），
> 于是 sm/md/lg 全部数成 0，我还顺着这个假信号往下推成了"移动端露馅的根因"。
> 用正确的串重测同一份产物：`sm:` 36 条、`md:` 12 条、`lg:` 4 条，
> 我新加的 `hidden sm:table-cell` 也在其中。断点系统正常工作，没有这个 bug。
>
> 留下两条教训：① 计数为 0 时先怀疑探针而不是被测物，尤其是转义/空白相关的匹配；
> ② 同一个结论换一种匹配方式对不上时，以能在产物里肉眼看到样例的那个为准
> （我当时其实看到过 `.md\:text-3xl`，却没因为这个推翻自己的统计）。
>
> 移动端那一行按实际状态记：管理端两张表都在 `overflow-x-auto` 里，
> 用户表的小屏列降级（登录方式、注册时间 `hidden sm:table-cell`）已生成到产物 CSS；
> 但**没有真实窄屏验证**（这台机器的内嵌浏览器 viewport 是 0×0，截不了图、媒体查询也测不了），
> 所以这一项算"代码与产物层面完成、视觉未验收"。
> —— 这条状态已被后面的「视觉验收（真浏览器）」一节推翻并补验：窄屏实测发现并修掉了一处真实溢出。
>
...
>
> **进度补记（2026-09-23，视觉验收：之前"截不了图"的结论是错的）**
>
> 前面有 6 处写着"这台机器截不了图、只能做 DOM 断言"。**那个限制只属于内置浏览器面板**（它的 viewport 是 0×0），
> 不属于这台机器：本机全局装了 Playwright 1.60 + Chromium（`%LOCALAPPDATA%\ms-playwright`），
> `npx playwright screenshot` 和脚本 API 都能正常出图。也就是说，前面几轮我"没做视觉回归"的地方，
> 不是做不了，是没去找工具 —— 这条教训比那几行状态更正要紧。
>
> 做法：`VITE_API_BASE=http://localhost:3001 vite build --outDir dist-qa` 出一份指向本地后端的构建
> （`.env.production` 默认指向还没部署的 Render，直接 build 出来的包每个接口都是红的），
> `vite preview` 挂在 **5173/3000 这两个 CORS 白名单里的端口**（挂在 4174 上会被后端拒，页面看着有骨架、数据全空），
> 然后 1280×900 与 390×844 两档逐页截图，同时记录文档级横向溢出、越界元素与控制台错误。
> 登录态要同时塞 `auth_token` + `auth_user`（只塞 token，store 水合后仍算未登录），管理端另塞 `admin_token`。
>
> 一轮下来抓到 4 个 DOM 断言抓不到的缺陷，全部已修：
> 1. **首页"已收录 27 篇文章 + 255 道题目"是写死的**（`Home.tsx` 两处）。数字今天碰巧对，语料一改就变成假宣传 ——
>    这正是 P0-2 清掉的那一类。新增 `GET /api/stats`（读同一份进程内缓存，不重新解析文件），
>    前端拿不到计数时整段不显示，而不是退回写死的值。实测接口返回 `{articles:27,questions:255}`，与旧文案一致。
> 2. **我的题库页在 390px 下横向溢出 4px**，右侧"全部难度"下拉被切掉一半。原因是两个 `<select>` 挤在
>    `flex items-center space-x-4` 里不换行也不收缩。改成 `flex-wrap` + `flex-1 min-w-0` + 最小宽度，
>    复测：`mobile/my-questions` 无横滚，两个下拉完整可见。
> 3. **六维雷达图的轴标签被画布裁掉**（"实操经验关联"只剩"实操经验"，右侧同理）。标签在 1.32 倍半径处向外排，
>    按 `0 0 260 260` 的 viewBox 算，最宽标签要伸到 274px。改成 `viewBox="-28 0 316 260"`，六个标签全部完整。
> 4. **自己的面试报告页写着"面试复盘报告（分享）/ 只读"**，而且控制台一条 404。根因是 `isAuthenticated`
>    由 store 异步水合，首帧为 false，于是先打了 `/api/interview/shared/:id`（私有会话必然 404）再退回自己的接口，
>    而两种来源共用一个"分享"标题。改为同步读 `getStoredToken()` 决定先后，并区分 `sharedByLink`：
>    自己的显示"我的报告"、去掉只读角标；分享链接保持原样。复测控制台无 404，标题正确。
>
> 顺带确认没问题的：管理端检索质量面板第一次看到真实渲染（14 条查询、拒答率 71.4%、平均 2938ms、分数分桶），
> 聊天页的引用角标 + 参考来源卡片、移动端汉堡菜单弹层、404 兜底页。
> 一处已知小瑕疵没修：答案里引用角标紧跟句号时，句号可能被挤到下一行开头（中文排版上不该出现），
> 属于排版细节，记在这里不装作没看见。
>
> 这轮也解释了之前一个误判：拿默认 `npm run build` 的产物做验收时，所有页面数据都是空的，
> 看起来像前端坏了，实际是 `.env.production` 指向未部署的远端后端。**验收口径要先把 API 指到能跑的后端。**
