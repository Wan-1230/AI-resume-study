...
export default function Skeleton({ className }: { className?: string }) {
  return <div className={cn('animate-pulse rounded-lg bg-[#1a1a22]', className)} aria-hidden="true" />;
}
  ...
  return (
    <div className="space-y-3" role="status" aria-label="加载中">
      {Array.from({ length: rows }, (_, i) => (
        <div key={i} className={cn('rounded-2xl border border-[#1e1e28] bg-[#141419] p-5', cardClassName)}>
